...
export class PRReviewCron {
  ...
  private async review_pr(
    ...
    const prompt_lines = [
      ...
      "If any check is failing or pending, do NOT merge.",
      "Instead, note the failing checks in your review and request changes.",
      "Skipped and neutral checks count as passing — our workflows use change",
      "detection, so a partial PR legitimately skips jobs.",
      "Only if this command prints nothing at all does the repo have no CI; in",
      "that case proceed with merge normally.",
      "",
