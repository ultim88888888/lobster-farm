...
- `worktree-lock.ts` -- The vocabulary of an agent worktree lock: how a lock reason is written, how our own locks are told apart from a human's, and how the owner token is parsed back out.
- `worktree-cleanup.ts` -- Worktree removal. Post-merge cleanup plus the guarded hourly sweep for stale worktrees. The only place a worktree is removed or a branch deleted.
- `worktree-lock-reaper.ts` -- Releases worktree locks whose owning session is gone, on the same hourly tick, immediately before the sweep. Releases locks and nothing else — it removes no worktree and deletes no branch.
...
- **Worktree lock lifecycle**: Every agent worktree is locked at creation (#360), because `git worktree remove` refuses a locked tree even with `--force` — that refusal is what stopped a bad staleness guess from destroying live work (#357), and since #358 a lock outranks even a confirmed merge. The cost is that a lock has no expiry of its own: sessions that finished, crashed, or were recycled left theirs behind until 35 of 48 worktrees in one repo were locked and nothing was cleanable (#370). So a lock now carries a machine-resolvable owner token (`[lf-owner:tmux:pool-3]`, `[lf-owner:pid:4242]`) and the reaper releases it once that owner is demonstrably gone and the directory has sat untouched for 2h — or, for a lock with no reason recorded at all, 24h. A foreign lock is never released at any age, including git's own `added with --lock`. Releasing returns a worktree to *normal* sweep eligibility; it never shortcuts a single one of the sweep's guards.
