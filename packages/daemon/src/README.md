...
- `index.ts` -- Entrypoint. Wires up all subsystems (registry, sessions, queue, features, Discord, Commander, pool) and handles graceful shutdown.
...
- `features.ts` -- `FeatureManager`. State machine for the feature lifecycle (plan, design, build, review, ship, done). Manages phase transitions, approval gates, agent spawning, and auto-advancement.
- `server.ts` -- HTTP API server. Routes for status, entities, features (CRUD + advance/approve), tasks, pool management, entity scaffolding, reload, and webhook endpoints.
- `router.ts` -- Discord message router. Deterministic rules: channel-type routing (alerts, work rooms, general), keyword-based intent classification. Legacy `!lf` prefix parsing retained for test coverage.
...
- `persistence.ts` -- JSON file persistence for feature state. Saves to and loads from `~/.lobsterfarm/state/features.json`.
- `pid.ts` -- PID file management. Write, read, remove, and check if the daemon is already running.

## Key Concepts

- **Feature lifecycle**: plan -> design (optional) -> build -> review -> ship -> done. Approval gates at plan and design. The feature manager auto-advances ungated phases when an agent session completes.
