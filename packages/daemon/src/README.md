...
- `check-suite-handler.ts` -- `check_suite.completed` dispatch. Drives the whole review/merge/fix loop for `pr_lifecycle: v2` entities; for v1 entities it does exactly one thing — merge a parked approval once its CI completes green (#355). This is the *fast* release path only: it fires once per SHA, and on a fast CI run that event usually arrives before the park was written, so it cannot be the only one (#372).
- `parked-approvals.ts` -- Resolver for parked v1 approvals (#372). Every 2 min it reads the parks out of `state/pr-reviews.json`, re-reads CI, and drives each to a terminal state: green → merge, red → alert and drop, PR closed or head moved → drop, still pending → wait and, past 30 min, escalate to #alerts as `action_required`. Restart-safe (the park is on disk, not in a timer) and structurally unable to merge past pending CI. Not a revival of pr-cron: it reads local state and touches GitHub only when a park exists.
